...
test_df["dev_test"] = test_df.apply (lambda x: x["model name"].split("_")[5], axis = 1)
test_df["model name"] = test_df.apply (lambda x: x["model name"].split(":")[0], axis = 1)
test_df["Model Number"] =  test_df["model name"].str.extract('_([0-9][abc])_')
...
dev_thresholds_figer = top_per_model.loc[top_per_model["dev_test"] == "fqadev112"][["EvalSet", "model name", "thesholds"]]
...
top_per_model_figer = withDevThresh.loc[withDevThresh["dev_test"] == "figerQA112"].loc[withDevThresh["thesholds"] == withDevThresh["dev thresholds"]]
